...
#feature set 4: feature set 1 + squared values of feature set 1
features <- scale(abs(as.matrix(data[,2:74]) - as.matrix(data[,75:ncol(data)]))); #accuracy: 0.7684
...
svm <- svmlight(trainingFeatures,trainingLabels,
                pathsvm='/Users/Cybelle/Documents/grad_school/school/classes/2015-2016_spring/cs498daf/svm_light')
...
svm <- svmlight(trainingFeatures,trainingLabels, svm.options = "-t 2 -g .02",
                pathsvm='/Users/Cybelle/Documents/grad_school/school/classes/2015-2016_spring/cs498daf/svm_light')
predictedLabels<-predict(svm, testFeatures) 
foo<-predictedLabels$class #"foo" = class labels (1 or 0) for each item in test set
#get classification accuracy:
accuracy<-sum(foo==testLabels)/(sum(foo==testLabels)+sum(!(foo==testLabels))) 
#accuracy w/ feature set 1, 1 partition: g = .1: .6624, 1: .5004, .01: 0.7956, .001: .7862, .015: .7966, .02: .7978, .03: .7956
#accuracy w/ feature set 2, 1 other partition: g = .02: .7818


#run linear SVM w/ regular + squared features (feature set 4)
...
foo<-predictedLabels$class #"foo" = class labels (1 or 0) for each item in test set
#get classification accuracy:
accuracy<-sum(foo==testLabels)/(sum(foo==testLabels)+sum(!(foo==testLabels))) #.785


#run radial SVM w/ regular + squared features (feature set 4)
svm <- svmlight(trainingFeatures,trainingLabels, svm.options = "-t 2 -g .02",
                pathsvm='/Users/Cybelle/Documents/grad_school/school/classes/2015-2016_spring/cs498daf/svm_light')
predictedLabels<-predict(svm, testFeatures) 
foo<-predictedLabels$class #"foo" = class labels (1 or 0) for each item in test set
#get classification accuracy:
accuracy<-sum(foo==testLabels)/(sum(foo==testLabels)+sum(!(foo==testLabels))) #accuracy = .7386
