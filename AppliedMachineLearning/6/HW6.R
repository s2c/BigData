...
#And then this should be a decent Rsquared
...
#extenData <- read.csv("default_plus_chromatic_features_1059_tracks.txt",header=F); # if we are using extended

#extenFts <- extenData[,-ncol(extenData):-(ncol(extenData)-1)]
#defaultFts <- defaultData[,-ncol(defaultData):-(ncol(defaultData)-1)]

#extenVal <- extenData[,(ncol(extenData)-1):ncol(extenData)]
#defaultVal <- defaultData[,(ncol(defaultData)-1):(ncol(defaultData))]

#latitude = 1
#longitude = 2

#Get the co-ord for default
#defLat <- defaultVal[,latitude]
#defLong <- defaultVal[,longitude]

#Get the co-ord for extended
#extenLat <- extenVal[,latitude]
#extenLong <- extenVal[,longitude]

#Create the Linear Model
#Is this correct??
#Something is wrong since Rquared=1 right now
#extenMod <- lm( extenLat ~ ) #Dont want to include longitude 


#summary(extenMod)

#Get the fitted values
#extenFittedLat <- fitted(extenMod)
#defaultFittedLat <- fitted(defaultMod)

#par(usr(min(extenFittedLat),max(extenFittedLat),min(extenLat),max(extenLat)))
#plot(extenFittedLat,extenLat,   xlim=c(min(extenFittedLat),max(extenFittedLat)),
#                               ylim = c(min(extenLat),max(extenLat))) 

###########################
#Cybelle added on 3/29/2016

#1, part 1: lattitude regression
