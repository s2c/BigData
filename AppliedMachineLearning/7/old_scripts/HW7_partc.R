...
s_range <- c(0.1,1,100,10000,100000,1000000);	#the range of scales to be used
...
for (day in 1:max(data$Julian)){  
  ...
  wmod<-cv.glmnet (as.matrix(day.df[,6:ncol(day.df)]) , as.vector ( day.df [ , 3 ] ) , alpha =1)
  ...
  preds[,day]<-predict.cv.glmnet(wmod, wmat, s='lambda.min' )
  
  #plot (wmod)
  #t h i s gets t he r e g u l a r i z a t i o n r e s u l t s
  
  #store model in array
  wmods[[day]] = wmod;
}

write.csv(preds,str_c("preds_part_c_alpha=",alpha,".csv"),row.names=F);
save(list=c("wmods","preds"),file=str_c("part_c_wmods_preds_alpha=",alpha));
...
#original code: 13.6528 secs -> /3 = 4.550933 per day
